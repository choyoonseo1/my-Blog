...
function App() {
  ...
  // 제목 정렬 (모든 배열 동기화 + currentIndex 재매핑)
  function handleSort() {
    const indices = title.map((_, i) => i).sort((a, b) =>
      title[a].localeCompare(title[b], 'ko')
    );

    setTitle(indices.map((i) => title[i]));
    setCreateDate(indices.map((i) => createDate[i]));
    setDetails(indices.map((i) => details[i]));
    setLike(indices.map((i) => like[i]));

    if (currentIndex !== null) {
      setCurrentIndex(indices.indexOf(currentIndex));
    }
  }
  ...
  return (
    <div className="App">
      <Title text="React + Vite로 만드는 블로그" />
      ...
      <Blog
        ...
        like={like}
        onSort={handleSort}
        onChangeFirstTitle={changeTitle}
        onAddLike={addLikes}
